...
export default function InboxPage() {
  ...
  if (loading) return <p className="text-body-sm text-[#6c757d]">Loading...</p>;

  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[#1a1a2e]">Contact Inbox</h1>
          <p className="mt-1 text-body-sm text-[#6c757d]">
            {items.length} messages · {unread} unread
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex gap-4 h-[calc(100vh-200px)]">
        ...
        <div className="w-80 shrink-0 overflow-y-auto rounded-xl bg-white shadow-sm divide-y divide-[#f0f0f0]">
          {items.length === 0 && (
            <p className="p-6 text-body-sm text-[#adb5bd] text-center">No submissions yet</p>
          )}
          {items.map((item) => (
            <button
              key={item.id}
            ...
            >
              ...
              <p className="mt-1 text-micro text-[#adb5bd]">{new Date(item.createdAt).toLocaleDateString()}</p>
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto rounded-xl bg-white shadow-sm p-6">
          {!selected ? (
            <div className="flex h-full items-center justify-center">
              <p className="text-body-sm text-[#adb5bd]">Select a message to read</p>
            </div>
          ) : (
            <div>
              <div className="flex items-start justify-between gap-4 pb-4 border-b border-[#f0f0f0]">
                <div>
                  <h2 className="text-lg font-semibold text-[#1a1a2e]">{selected.subject || '(No subject)'}</h2>
                  <p className="mt-1 text-body-sm text-[#6c757d]">
                    From: <span className="font-medium text-[#1a1a2e]">{selected.name}</span>
                    {selected.company && ` · ${selected.company}`}
                    {' · '}<a href={`mailto:${selected.email}`} className="text-gold-500 hover:underline">{selected.email}</a>
                  </p>
                  <p className="text-caption text-[#adb5bd] mt-1">{new Date(selected.createdAt).toLocaleString()}</p>
                </div>
                <button
