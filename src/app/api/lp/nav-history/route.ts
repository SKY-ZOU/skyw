/**
 * GET /api/lp/nav-history
 * 查询基金净值历史
 *
 * Query params:
 * - fund_id: string (required, or mode=comparison)
 * - limit: number (default 24)
 * - mode: 'comparison' — 返回各基金多时间段累计回报
 */

import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { lpAuthOptions } from '@/lib/lp-auth-options'
import { createAdminClient } from '@/lib/lp-supabase'

export async function GET(request: NextRequest) {
  try {
    const session = await getServerSession(lpAuthOptions)
    if (!session?.user) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const { searchParams } = new URL(request.url)
    const fundId = searchParams.get('fund_id')
    const limit = parseInt(searchParams.get('limit') || '24', 10)
    const mode = searchParams.get('mode')

    const supabase = createAdminClient()

    // 对比模式：返回各基金的多时间段累计回报
    if (mode === 'comparison') {
      const { data: funds, error: fundsError } = await supabase
        .from('funds')
        .select('id, name')
        .eq('status', 'active')

      if (fundsError || !funds) {
        return NextResponse.json({ error: '获取基金列表失败' }, { status: 500 })
      }

      const results = await Promise.all(
        funds.map(async (fund: any) => {
          const { data: history } = await supabase
            .from('nav_history')
            .select('date, nav')
            .eq('fund_id', fund.id)
            .order('date', { ascending: false })
            .limit(400)

          if (!history || history.length === 0) {
            return { id: fund.id, name: fund.name, return_1m: 0, return_3m: 0, return_6m: 0, return_1y: 0 }
          }

          const sorted = [...history].sort((a, b) => b.date.localeCompare(a.date))
          const latest = sorted[0]

          const calcReturn = (daysAgo: number) => {
            const targetDate = new Date(latest.date)
            targetDate.setDate(targetDate.getDate() - daysAgo)
            const targetStr = targetDate.toISOString().split('T')[0]
            const ref = sorted.find(h => h.date <= targetStr)
            if (!ref) return 0
            return ((latest.nav - ref.nav) / ref.nav) * 100
          }

          return {
            id: fund.id,
            name: fund.name,
            return_1m: parseFloat(calcReturn(30).toFixed(2)),
            return_3m: parseFloat(calcReturn(90).toFixed(2)),
            return_6m: parseFloat(calcReturn(180).toFixed(2)),
            return_1y: parseFloat(calcReturn(365).toFixed(2)),
          }
        })
      )

      return NextResponse.json({ success: true, data: results })
    }

    // 单基金净值历史
    if (!fundId) {
      return NextResponse.json({ error: 'fund_id 为必填项' }, { status: 400 })
    }

    const { data: history, error } = await supabase
      .from('nav_history')
      .select('date, nav')
      .eq('fund_id', fundId)
      .order('date', { ascending: true })
      .limit(limit)

    if (error) {
      return NextResponse.json({ error: '查询失败: ' + error.message }, { status: 500 })
    }

    // 计算累计收益率（相对第一条）
    const enriched = (history || []).map((item: any, idx: number) => {
      const base = history![0]?.nav || item.nav
      return {
        date: item.date,
        nav: item.nav,
        return_pct: parseFloat(((item.nav - base) / base * 100).toFixed(2))
      }
    })

    return NextResponse.json({ success: true, data: enriched })

  } catch (error) {
    console.error('nav-history error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
